...
> This file mirrors `AGENTS.md` (canonical source — keep the two in sync).
...
- Periods are scoped to an event: `POST /api/events/{id}/periods` returns 404 for an unknown event, rejects overlapping dates per user **within that event** (409), and validates color in `green|red`, dates in `YYYY-MM-DD`, `start <= end`. `DELETE /api/periods/{id}` and `POST /api/login` are event-agnostic.
